# NorthCoast Laser MedSpa — Mentor, OH

## Overview
- **Website:** https://www.northcoastlaser.com
- **Phone:** (440) 639-3223
- **Address:** 9500 Mentor Ave, Ste 130, Mentor, OH 44060
- **EMR/Booking:** TBD (listed on Allē — uses Allergan products)
- **Independent:** Yes — single location
- **CareCredit accepted** — indicates established practice

## Owner / Key Staff
- Key providers: Melissa, Betsy (mentioned in reviews)
- Owner TBD (need further research)

## Services
- Botox / Neurotoxins
- Dermal Fillers
- Laser Hair Removal
- Laser Skin Treatments
- Body Treatments

## Social Media
- TBD

## Hours
- Mon-Fri: 9AM-5PM
- Sat: 9AM-12PM
- Sun: Closed

## Notes
- Mentor is east-side Cleveland suburb, Lake County — ~50 min from Brunswick.
- Phone prominently displayed. Good Yelp presence (41 photos, active reviews updated March 2026).
- On Allē (Allergan loyalty program) — indicates they're an authorized Allergan provider (Botox, Juvederm).
- CareCredit partner — established, professional operation.
- Saturday half-day = missed calls on weekends likely.
- "Treated like family" positioning — relationship-focused, would value lead recovery.

## Estimated Revenue
- Single location, full laser + injectables menu, CareCredit → est. $400K-$800K/year

## Prospect Score: 8/10
- ✅ Independent, single location
- ✅ Phone prominent, good online presence
- ✅ Established (Allē, CareCredit, active reviews)
- ✅ Injectables + laser (high-value)
- ✅ Saturday-only half day = after-hours opportunity
- ⚠️ Not on Moxie (manual handoff initially)

## Lead Leak Check — March 10, 2026

**Phone (verified):** (440) 639-3223 ✅ confirmed on website
**Owners:** Betsy (Licensed Aesthetician), James A. Turbett, MD (Medical Director), Melissa (Nurse Injector) — trio
**Google:** 300+ reviews per website claim ("400+ Five Star Reviews" including non-Google)
**Website:** Webflow-based. NO online booking — "Schedule" button is just a tel: link. $50-off lead capture form at bottom.
**Instagram:** @northcoast_lasermedspa — 1,177 followers, 1,089 posts

### Findings
1. **Contact form is BROKEN** — the $50-off lead capture form shows "Oops! Something went wrong while submitting the form." Active lead leak RIGHT NOW. ([northcoastlaser.com — scroll to bottom](https://www.northcoastlaser.com/))
2. **"Schedule an Appointment" is just a phone call link** — no actual online scheduling. With 300+ Google reviews they're busy, but every booking requires a call during business hours. ([northcoastlaser.com](https://www.northcoastlaser.com/))
3. **Saturday half-day (9AM-12PM) + no text-back = weekend lead leak** — unreachable 75% of the weekend. Competitors with online booking capture those leads. ([northcoastlaser.com](https://www.northcoastlaser.com/))
