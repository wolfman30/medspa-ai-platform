...
// registerAdminRoutes mounts JWT-protected admin endpoints: clinic management,
// messaging, onboarding, stories, prospects, briefs, finance, and testing.
func registerAdminRoutes(r chi.Router, cfg *Config) {
	...
	r.Route("/admin", func(admin chi.Router) {
		...
		registerAdminBriefs(admin, cfg)
		registerAdminFinance(admin, cfg)
		registerAdminProspects(admin, cfg)
		registerAdminStories(admin, cfg)
		registerAdminOnboarding(admin, cfg)
		registerAdminClinicRoutes(admin, cfg)
		registerAdminDashboard(admin, cfg)
	})
}

// registerAdminBriefs mounts morning briefs CRUD endpoints.
func registerAdminBriefs(admin chi.Router, cfg *Config) {
...
// registerAdminFinance mounts finance dashboard and research intelligence endpoints.
func registerAdminFinance(admin chi.Router, cfg *Config) {
...
// registerAdminProspects mounts prospect tracker and Rule-of-100 endpoints.
func registerAdminProspects(admin chi.Router, cfg *Config) {
...
// registerAdminStories mounts Kanban story board CRUD endpoints.
func registerAdminStories(admin chi.Router, cfg *Config) {
...
// registerAdminOnboarding mounts clinic creation and prefill endpoints.
func registerAdminOnboarding(admin chi.Router, cfg *Config) {
...
// registerAdminClinicRoutes mounts per-clinic admin endpoints: config, knowledge,
// stats, conversations, data purge, and payment provider integration.
...
// registerAdminDashboard mounts dashboard and manual testing tracker endpoints.
func registerAdminDashboard(admin chi.Router, cfg *Config) {
	if cfg.DB == nil {
		return
	}
