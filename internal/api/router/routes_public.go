...
// registerPublicRoutes mounts all unauthenticated endpoints: webhooks, health
// checks, OAuth callbacks, client self-service, and onboarding flows.
func registerPublicRoutes(r chi.Router, cfg *Config) {
	r.Group(func(public chi.Router) {
		...
		if cfg.AdminOnboarding != nil {
			registerPublicOnboardingRoutes(public, cfg)
		}
	})
}

// registerPublicOnboardingRoutes mounts self-service onboarding endpoints
// that are protected by the onboarding token rather than admin JWT.
func registerPublicOnboardingRoutes(public chi.Router, cfg *Config) {
