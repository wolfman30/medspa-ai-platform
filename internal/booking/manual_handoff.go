...
func FormatLeadSummaryHTML(lead LeadSummary) string {
	...
	var notesRow string
	if lead.ConversationNotes != "" {
		notesRow = fmt.Sprintf(`<tr><td style="padding:6px 12px;font-weight:bold;">Notes</td><td style="padding:6px 12px;">%s</td></tr>`, lead.ConversationNotes)
	}
	var emailRow string
	if lead.PatientEmail != "" {
		emailRow = fmt.Sprintf(`<tr><td style="padding:6px 12px;font-weight:bold;">Email</td><td style="padding:6px 12px;">%s</td></tr>`, lead.PatientEmail)
	}
	var scheduleRow string
	if schedule != "" {
		scheduleRow = fmt.Sprintf(`<tr><td style="padding:6px 12px;font-weight:bold;">Schedule Preference</td><td style="padding:6px 12px;">%s</td></tr>`, schedule)
	}
...
<p style="color:#666;font-size:12px;">This lead was qualified by AI assistant. Please reach out to confirm the appointment.</p>
		...
		valueOrNA(lead.PatientName),
		lead.PatientPhone, valueOrNA(lead.PatientPhone),
		emailRow,
		valueOrNA(lead.ServiceRequested),
		valueOrNA(lead.PatientType),
		scheduleRow,
